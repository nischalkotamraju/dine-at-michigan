...
const NotificationItem = ({
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'Food Alert':
        return <ChefHat size={20} color={COLORS['um-maize']} />;
      case 'Location Update':
        return <MapPin size={20} color={COLORS['um-maize']} />;
      case 'Special Alert':
        return <Star size={20} color={COLORS['um-maize']} />;
      case 'System Announcement':
        return <Bell size={20} color={COLORS['um-maize']} />;
      default:
        return <Bell size={20} color={COLORS['um-maize']} />;
    }
  };
        ...
        <View className="flex-1">
          <View className="flex-row items-center justify-between">
            ...
            <View className="flex-row items-center gap-x-2">
              {notification.redirect_url && (
                <ExternalLink size={14} color={COLORS['um-maize']} />
              )}
          ...
          {!notification.isRead && (
            <View className="mt-2 flex-row">
              <View className="rounded-full bg-um-maize px-2 py-1">
        ...
        <FlashList
                  ...
                  {unreadCount > 0 && (
                    <View className="flex-row items-center gap-x-3">
                      <TouchableOpacity
                        ...
                        className="rounded-lg bg-um-maize px-3 py-1"
                      >
                        <Text className="font-semibold text-white text-xs">Read All</Text>
                      </TouchableOpacity>
                      <View className="rounded-full bg-um-maize px-3 py-1">
          ...
          ListEmptyComponent={
            <View className="mt-12 flex items-center justify-center px-6">
              <Text className="mt-4 font-bold text-lg text-um-maize">
